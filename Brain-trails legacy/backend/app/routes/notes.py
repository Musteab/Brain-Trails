...
from ..models import Note, NoteFolder, NoteTag, NoteTagLink, Quiz, Question, QuizSourceLink
...
def create_note():
    ...
    if len(title) > 120:
        return json_response(error={"code": "INVALID_INPUT", "message": "Title too long"}, status=400)
    if not isinstance(content, dict):
        return json_response(error={"code": "INVALID_INPUT", "message": "Content must be a document object"}, status=400)
    tag_ids = payload.get("tag_ids", [])
    
    # Extract plaintext for search
    plaintext = extract_plaintext(content)
    max_chars = current_app.config.get("AI_MAX_INPUT_CHARS", 4000)
    if plaintext and len(plaintext) > max_chars:
        return json_response(
            error={"code": "CONTENT_TOO_LARGE", "message": f"Content exceeds {max_chars} characters"},
            status=400,
        )
...
def update_note(note_id: int):
    ...
    # Update fields
    if "title" in payload:
        new_title = (payload["title"] or "Untitled").strip()
        if len(new_title) > 120:
            return json_response(
                error={"code": "INVALID_INPUT", "message": "Title too long"},
                status=400,
            )
    ...
    if "content" in payload:
        ...
        max_chars = current_app.config.get("AI_MAX_INPUT_CHARS", 4000)
        if note.plaintext_cache and len(note.plaintext_cache) > max_chars:
            return json_response(
                error={"code": "CONTENT_TOO_LARGE", "message": f"Content exceeds {max_chars} characters"},
                status=400,
            )
...
@jwt_required()
def generate_note_summary(note_id: int):
    """
    Generate an AI summary of a note's content.
    
    Body:
    {
        "max_sentences": 3  (optional, default 3, max 10)
    }
    ...
    payload = request.get_json() or {}
    max_sentences = min(int(payload.get("max_sentences", 3)), 10)
    
    # Get plain text from note content
    text = note_to_prompt_text(note.content)
    
    if not text or len(text) < 50:
        return json_response(
            error={"code": "INSUFFICIENT_CONTENT", "message": "Note content is too short to summarize"},
            status=400
        )
    ...
    try:
        ...
        if not summary:
            return json_response(
                error={"code": "GENERATION_FAILED", "message": "Unable to generate summary"},
                status=500
            )
        ...
        current_app.logger.exception("Summary generation failed: %s", e)
        return json_response(
            error={"code": "GENERATION_FAILED", "message": "Failed to generate summary"},
            status=500
        )
...
def list_folders():
    """
    Get all folders for the current user as a tree structure.
    Returns root folders with nested children.
    """
    user_id = int(get_jwt_identity())
    
    # Get root folders (no parent)
    root_folders = NoteFolder.query.filter_by(
        user_id=user_id,
        parent_id=None
    ).order_by(NoteFolder.position, NoteFolder.name).all()
    
    return json_response([folder.to_dict(include_children=True) for folder in root_folders])
...
def create_folder():
    ...
    name = (payload.get("name") or "").strip()
    if not name:
        return json_response(error={"code": "INVALID_INPUT", "message": "Folder name is required"}, status=400)
    
    if len(name) > 128:
        return json_response(error={"code": "INVALID_INPUT", "message": "Folder name too long (max 128)"}, status=400)
    
    parent_id = payload.get("parent_id")
    
    # Validate parent exists and belongs to user
    if parent_id:
        parent = NoteFolder.query.filter_by(id=parent_id, user_id=user_id).first()
        if not parent:
            return json_response(error={"code": "NOT_FOUND", "message": "Parent folder not found"}, status=404)
    ...
    if existing:
        return json_response(error={"code": "DUPLICATE", "message": "Folder with this name already exists"}, status=409)
    ...
    return json_response(folder.to_dict()), 201


@notes_bp.route("/folders/<int:folder_id>", methods=["GET"])
@jwt_required()
def get_folder(folder_id: int):
    """Get a folder with its contents (subfolders and notes)."""
    user_id = int(get_jwt_identity())
    
    folder = NoteFolder.query.filter_by(id=folder_id, user_id=user_id).first()
    if not folder:
        return json_response(error={"code": "NOT_FOUND", "message": "Folder not found"}, status=404)
    
    return json_response({
        "folder": folder.to_dict(include_children=True, include_notes=True),
        "path": folder.get_path(),
    })


@notes_bp.route("/folders/<int:folder_id>", methods=["PATCH"])
@jwt_required()
def update_folder(folder_id: int):
    ...
    if "name" in payload:
        ...
        if not name:
            return json_response(error={"code": "INVALID_INPUT", "message": "Folder name is required"}, status=400)
        if len(name) > 128:
            return json_response(error={"code": "INVALID_INPUT", "message": "Folder name too long"}, status=400)
        
        # Check for duplicate
        existing = NoteFolder.query.filter(
    ...
    if "parent_id" in payload:
        ...
        if new_parent_id == folder_id:
            return json_response(error={"code": "INVALID_INPUT", "message": "Cannot move folder into itself"}, status=400)
        
        if new_parent_id:
            # Check parent exists
            parent = NoteFolder.query.filter_by(id=new_parent_id, user_id=user_id).first()
            if not parent:
                return json_response(error={"code": "NOT_FOUND", "message": "Parent folder not found"}, status=404)
            
            # Check not moving into descendant
            current = parent
            while current:
                if current.id == folder_id:
                    return json_response(error={"code": "INVALID_INPUT", "message": "Cannot move folder into its descendant"}, status=400)
    ...
    return json_response(folder.to_dict())
...
def delete_folder(folder_id: int):
    """
    Delete a folder.
    Notes in the folder will be moved to the parent folder (or root if no parent).
    ...
    Note.query.filter_by(folder_id=folder_id).update({"folder_id": folder.parent_id})
    
    # Delete folder (cascade will delete subfolders)
    db.session.delete(folder)
    db.session.commit()
    
    return json_response({"message": "Folder deleted"})


@notes_bp.route("/<int:note_id>/move", methods=["PATCH"])
@jwt_required()
def move_note(note_id: int):
    ...
    Body:
    {
        "folder_id": null | folder_id  (null = root/unfiled)
    }
    """
    user_id = int(get_jwt_identity())
...
@notes_bp.route("/unfiled", methods=["GET"])
@jwt_required()
def list_unfiled_notes():
