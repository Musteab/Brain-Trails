...
export const flashcardApi = {
  ...
  exportDeck: (deckId, format = 'json') => client.get(`/decks/${deckId}/export`, { 
    params: { format },
    responseType: format === 'csv' ? 'blob' : 'json'
  }),
  
  /** Import cards into existing deck (JSON) */
  importCards: (deckId, cards) => client.post(`/decks/${deckId}/import`, { cards }),
  
  /** Import CSV file into existing deck */
  importCSV: (deckId, file) => {
    const formData = new FormData();
    formData.append('file', file);
    return client.post(`/decks/${deckId}/import`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
  },
  
  /** Import a new deck with cards */
  importDeck: (data) => client.post('/decks/import', data),
};
...
export const settingsApi = {
  ...
  updatePreferences: (payload) => client.patch('/settings/preferences', payload),
  
  /** Update preferences by category */
  updateCategory: (category, payload) => client.patch(`/settings/preferences/category/${category}`, payload),
  
  /** Export all user data */
  exportData: () => client.get('/settings/export'),
  
  /** Reset all progress (dangerous!) */
  resetProgress: () => client.delete('/settings/progress'),
  
  /** Delete account (dangerous!) */
  deleteAccount: () => client.delete('/settings/account'),
  
  /** Get active sessions */
  getSessions: () => client.get('/settings/sessions'),
  
  /** Logout from all devices */
  logoutAll: () => client.post('/settings/logout-all'),
};
...
export const notesApi = {
  ...
  generateSummary: (noteId, maxSentences = 3) => client.post(`/notes/${noteId}/summary`, { max_sentences: maxSentences }),
  ...
  move: (noteId, folderId) => client.patch(`/notes/${noteId}/move`, { folder_id: folderId }),
  
  /** Get unfiled notes (not in any folder) */
  getUnfiled: () => client.get('/notes/unfiled'),
  ...
  // Folders
  listFolders: () => client.get('/notes/folders'),
  getFolder: (folderId) => client.get(`/notes/folders/${folderId}`),
  createFolder: (payload) => client.post('/notes/folders', payload),
  updateFolder: (folderId, payload) => client.patch(`/notes/folders/${folderId}`, payload),
  deleteFolder: (folderId) => client.delete(`/notes/folders/${folderId}`),
};
