...
const FlashcardsPage = () => {
  ...
  const importCardsMutation = useMutation({
    mutationFn: (cards) => flashcardApi.importCards(selectedDeck, cards),
    onSuccess: () => {
      queryClient.invalidateQueries(['flashcards', selectedDeck]);
      queryClient.invalidateQueries(['decks']);
    ...
    onError: (error) => {
      setImportError(error.response?.data?.error || 'Import failed');
    }
  });
  ...
  const handleExportJSON = async () => {
    ...
    try {
      const response = await flashcardApi.exportDeck(selectedDeck, 'json');
      const data = response.data;
      const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${currentDeckName.replace(/\s+/g, '_')}_flashcards.json`;
      a.click();
  ...
  const handleExportCSV = async () => {
    if (!selectedDeck) return;
    try {
      const response = await flashcardApi.exportDeck(selectedDeck, 'csv');
      const url = URL.createObjectURL(response.data);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${currentDeckName.replace(/\s+/g, '_')}_flashcards.csv`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error('Export failed:', error);
  ...
  const handleImportSubmit = () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        throw new Error('Invalid format. Expected array of cards or {cards: [...]}');
      }
      
      if (!cards.length) {
        setImportError('No cards found in the import data');
        return;
      }
      
      importCardsMutation.mutate(cards);
    } catch (e) {
  ...
  const handleCSVFileImport = async (event) => {
    ...
    try {
      await flashcardApi.importCSV(selectedDeck, file);
      queryClient.invalidateQueries(['flashcards', selectedDeck]);
      queryClient.invalidateQueries(['decks']);
      setImportModalOpen(false);
    } catch (error) {
      setImportError(error.response?.data?.error || 'CSV import failed');
    }
    event.target.value = '';
  };
  ...
  return (
    <Stack spacing={3}>
                ...
                {selectedDeck && (
                  <>
                    ...
                    >
                      ...
                      <MenuItem onClick={() => { setDeckMenuAnchor(null); setImportModalOpen(true); }}>
      ...
      <Dialog 
        ...
        onClose={() => { setImportModalOpen(false); setImportError(null); setImportText(''); }}
        maxWidth="sm"
        fullWidth
      >
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Paste JSON data or upload a CSV file. JSON format:
            </Typography>
            <Typography 
              component="pre" 
              sx={{ 
...
{`[
  ...
  {"question": "Q2", "answer": "A2"}
        ...
        <DialogActions>
          <Button onClick={() => { setImportModalOpen(false); setImportError(null); setImportText(''); }}>
            Cancel
          </Button>
          <Button 
            variant="contained" 
            onClick={handleImportSubmit}
            disabled={!importText.trim() || importCardsMutation.isPending}
          >
            {importCardsMutation.isPending ? 'Importing...' : 'Import JSON'}
          </Button>
        </DialogActions>
      </Dialog>
    </Stack>
