...
export default function ProfilePage() {
  ...
  const { data: heatmapData, isLoading: heatmapLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get('/profile/heatmap');
      return res.data;
    },
  });
  ...
  const { data: activityData, isLoading: activityLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/profile/activity?page=${activityPage}&per_page=10`);
      return res.data;
    },
  });
  ...
  const { data: petsData, isLoading: petsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get('/profile/pets');
      return res.data;
    },
  });
  ...
  const togglePublicMutation = useMutation({
    mutationFn: async (isPublic) => {
      const res = await api.post('/profile/public', { is_public: isPublic });
      return res.data;
    },
    onSuccess: () => {
