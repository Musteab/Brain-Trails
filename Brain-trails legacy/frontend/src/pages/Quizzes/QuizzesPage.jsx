...
const QuizRunner = ({ quiz, onSubmit, result, resetQuiz }) => {
  ...
  const checkTextAnswer = (question) => {
  ...
  const getQuestionType = (question) => {
    ...
    // Infer from options
    if (!question.options || question.options.length === 0) return 'short_answer';
    if (question.options.length === 2 && 
        question.options.map(o => o.toLowerCase()).sort().join(',') === 'false,true') {
      return 'true_false';
    }
    return 'mcq';
  };
            ...
            return (
              ...
              >
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1}>
                  ...
                  <Chip 
                    size="small" 
                    label={questionType === 'true_false' ? 'T/F' : questionType === 'short_answer' ? 'Short' : 'MCQ'}
                    variant="outlined"
                  />
                </Stack>
                ...
                {(questionType === 'mcq' || questionType === 'true_false') && (
                  <Stack spacing={1} direction={questionType === 'true_false' ? 'row' : 'column'}>
                    {(question.options || []).map((option) => {
                      ...
                      return (
                        <Button
                          ...
                          sx={{
                            justifyContent: questionType === 'true_false' ? 'center' : 'flex-start',
                            flex: questionType === 'true_false' ? 1 : 'none',
                            bgcolor: bg,
                            color: 'text.primary',
                ...
                {questionType === 'short_answer' && (
                  <Stack spacing={1}>
                    <TextField
                      ...
                      onBlur={() => checkTextAnswer(question)}
                      onKeyDown={(e) => e.key === 'Enter' && checkTextAnswer(question)}
                    ...
                    {!state && responses[question.id] && (
                      <Button 
                        size="small" 
                        variant="outlined"
                        onClick={() => checkTextAnswer(question)}
                      >
                        Check Answer
                      </Button>
                    )}
                ...
                {state && (
                  <Chip
                    sx={{ mt: 1 }}
                    label={state === 'correct' ? 'Correct!' : `Answer: ${question.correct_answer}`}
                    color={state === 'correct' ? 'success' : 'warning'}
                  />
                )}
                
                {/* Show explanation if available and answered */}
