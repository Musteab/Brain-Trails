...
def get_dashboard_summary():
    ...
    try:
        ...
        # Pet mood
        pet_mood = calculate_pet_mood(user_id)
        
        return jsonify({
            "ok": True,
            "data": {
                "today": {
                    "minutes": 0,  # Would come from gamification
                    "goal_minutes": 120,
                    "xp_today": 0,
                    "streak": 0
                },
    ...
    try:
        ...
        if active_pet:
            pet_data = {
                ...
                "emoji": get_pet_emoji(active_pet.pet_type),
                "mood": active_pet.mood or "happy",
                "level": active_pet.level,
                "xp": active_pet.xp,
                "xp_to_next": active_pet.xp_to_next_level,
                "xp_percent": round((active_pet.xp / max(active_pet.xp_to_next_level, 1)) * 100),
            }
        else:
            # Default pet for users without one
            pet_data = {
                "id": None,
                "pet_type": "forest-sprite",
                "name": "Willow",
                "emoji": "🦊",
