...
import { Lightbulb, AlertTriangle, BookOpen, type LucideIcon } from 'lucide-react';
...
const CalloutComponent = (props: any) => {
  ...
  let iconColor = 'text-slate-500 dark:text-slate-300';
  ...
  return (
    <NodeViewWrapper className={`my-4 p-4 rounded-xl border-2 flex gap-3 items-start ${bgClass}`}>
      <div className={`select-none mt-1 ${iconColor}`}><Icon className="w-5 h-5" /></div>
